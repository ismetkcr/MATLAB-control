function [sys,x0,str,ts] = contAPID_RLS(t,x,u,flag)
...
%
x0  = [0 0 0 0 0 0 0 20 1/20 5 100 0 0 0 100 0 0 0 100 0 0];
...
cikis_hesap_n_1 = x(20);
cikis_hesap_n_2 = x(21);
...
%en_n = (mu*v*error_n)/(mu + v*abs(error_n));
%en_n = (mu*v*error_n)/sqrt(mu^2 + v^2*error_n^2);
...
deltaen_n_2 = en_n_2 - en_n_3;
deltaen_n = en_n - en_n_1;
...
fi_n_1 = [en_n_2, epsilon_n_2, deltaen_n_2];
fi_n = [en_n, epsilon_n, deltaen_n];

A = 1 + fi_n_1*P_n_1*transpose(fi_n_1);
B = P_n_1*transpose(fi_n_1)*fi_n_1*P_n_1;

P_n = P_n_1 - ((1/A)*B);

C = (en_n_1 - cikis_hesap_n_2 + (fi_n_1*theta_n_1));
D = P_n * transpose(fi_n_1);

theta_n = theta_n_1 + (C*D);

cikis_hesap_n = fi_n * theta_n;
cikis_n = (cikis_hesap_n);
...
state(18) = P_n(6);
state(19) = P_n(9);
state(20) = cikis_hesap_n;
state(21) = cikis_hesap_n_1;
