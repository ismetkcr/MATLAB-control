...
function [sys,x0,str,ts] = filter_Update_cont(t,x,u,flag,ksi0, n0, K, threshold, I_inc, I_dec)
...
multip1 = (-2*(r_k - y_m_k)*b1) + 2*K*(u_k - u_k_1);
multip2 = u_k_1 - ((v_k - a1*v_k_1 - a2*v_k_2)/b1);
multip3 = filter_param*(1-filter_param);
multip4 = n_k_1;

tol = 10e-4;
J_k = ((r_k - y_m_k)^2) + K*((u_k - u_k_1)^2);
diff_J = (J_k - J_k_1);
if abs(diff_J)<tol || J_k<tol
    diff_J = 0;
    J_k = 0;
end
diff_J
...
if diff_J > threshold
    ksi_n = ksi_n_1;
    n_k = I_dec*n_k_1;
elseif diff_J <= threshold && diff_J>0
    ksi_n = ksi_n_1 - (multip1*multip2*multip3*multip4);
    n_k = n_k_1;
elseif diff_J < 0
    ksi_n = ksi_n_1 - (multip1*multip2*multip3*multip4);
    n_k = I_inc * n_k_1;
