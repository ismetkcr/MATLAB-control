function [sys,x0,str,ts] = Analog_Output(t,x,u,flag, OC1, OC2, OC3, OC4)
...
if isempty(s) 
    ...
    i = 0;
    if OC1==1
        cmd = sprintf('%s%+07.3f','#030',0);
        fprintf(s,'%s\r',cmd);
        fscanf(s);
    end    
    if OC2==1
        cmd = sprintf('%s%+07.3f','#031',0);
        fprintf(s,'%s\r',cmd);
        fscanf(s);
    end    
    if OC3==1
        cmd = sprintf('%s%+07.3f','#032',0);
        fprintf(s,'%s\r',cmd);
        fscanf(s);
    end    
    if OC4==1
        cmd = sprintf('%s%+07.3f','#033',0);
        fprintf(s,'%s\r',cmd);
        fscanf(s);
    end    
end
